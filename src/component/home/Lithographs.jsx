...
class Lithographs extends React.Component {
   constructor(){
      super();
   }

   componentDidMount(){
      this.props.getData('lithographs');
      //console.log("=====Stateprops",this.props);
   } 
   render(props) {
      let {lithographsdata}= this.props;
      let lithographs;
      if(lithographsdata.payload !==undefined){
         lithographs=lithographsdata.payload.data;
      }     
      return (
         <div> 
            <section className="Art">
               <div className="container">
                  <h2 className="headings-all">Lithographs / Serigraphs</h2>   
                  <div className="border"></div>
                     <div className="row">
                     {(typeof lithographs !=='undefined')?lithographs.map(item => (
...
const mapStateToProps =(state)=>{
   //alert("mapStateToProps");
	return{
      lithographsdata:  state.lithographs?state.lithographs:[]
	}
}
...
const mapDispatchToProps =(dispatch)=>{
 ...
 export default connect(mapStateToProps,mapDispatchToProps) (Lithographs);